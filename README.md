...
4. **Конструктор** — преглед на секции и въпроси; редакция на **заглавие на анкетата**, **заглавия на секции** и **въпроси** (вкл. изтриване); **настройки на теста**; „Още въпроси“ по секция; **Завърши генерирането** — анкетата става достъпна за стартиране. Същите операции са налични и през **REST API** (вижте [REST API](#rest-api-sanctum) и `/api-docs`).
...
| Функция | Описание |
|---------|----------|
| **Търсене / филтър** | Query параметри `q` (текст по заглавие на анкетата, ключови думи или **заглавие на секция**) и `status` (чернова, избор на заглавие, в изграждане, завършена). |
...
| Създаване и AI | `POST /api/questionnaires` → заглавия; `POST …/select-title` → секции/въпроси; `GET …/build`, `PATCH …/title` (заглавие на анкетата), `PATCH/DELETE …/sections/{id}`, `PATCH/DELETE …/questions/{id}`, `POST …/generate-more`, `POST …/settings`, `POST …/finish` |
...
Във всички защитени заявки изпращайте заглавие **`Authorization: Bearer &lt;token&gt;`** и по желание **`Accept: application/json`**. Анкетите в URL се адресират по **`uuid`**, не по числов `id`. **Секциите и въпросите** в пътищата за редакция се адресират с **числови `id`** от отговора на `GET …/build`. Маршрутите са дефинирани в `routes/api.php`; контролерите — под `app/Http/Controllers/Api/`.
...
app/
  Http/Controllers/
    ...
    QuestionnaireController.php    # CRUD, филтри, duplicate, export, настройки, генериране, редакция на заглавие/секции/въпроси
