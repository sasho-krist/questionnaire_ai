...
- [Потребители, достъп и верификация](#потребители-достъп-и-верификация)
...
За **верификация на имейл** и **връзка за нова парола** приложението изпраща транзакционни писма. Локално е удобно `MAIL_MAILER=log` (писмата се записват в лога).
...
Има миграция за **попълване на `email_verified_at`** за вече съществуващи потребители (преди въвеждане на задължителна верификация), за да не останат блокирани след ъпгрейд.
...
## Потребители, достъп и верификация

- **Регистрация** (`/register`) и **вход** (`/login`). При вход има **rate limit** (throttle) срещу brute force; неуспешните опити се **логват**.
- След регистрация се изисква **потвърждение на имейл** (линк в писмо). Докато имейлът не е потвърден, маршрутите за анкети са недостъпни (`verified` middleware).
- **Забравена парола:** `/forgot-password` и страница за нова парола с токен от имейл.
- **Изход** — пренасочване към началната страница.

За да работят писмата, вижте [Имейл: SMTP и Resend](#имейл-smtp-и-resend).
...
1. **Регистрация и потвърждение на имейл** — след регистрация отворете връзката в писмото (или „Изпрати отново“ на страницата за верификация).
...
В **конструктора** за **завършена** анкета има **публичен линк за старт** (копиране) и **QR код** към същия адрес (външна услуга за генериране на изображение). Достъпът до старт/попълване изисква **влезъл и с потвърден имейл** потребител (маршрутите са под `auth` + `verified`).
